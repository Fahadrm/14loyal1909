...
class grn_report_view(models.TransientModel):
    _name = "grn.report.view"
    _description = "GRN Report View"

    origin = fields.Char('PO Number', required=True)
    name = fields.Char('Reference', required=True)
    partner = fields.Many2one('res.partner', required=True)
    effective_date = fields.Datetime('Received Date', required=True)
    productname = fields.Many2one('product.product', required=True)
    demanded_qty = fields.Float('Demanded Qty', required=True)
