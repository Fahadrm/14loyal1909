...
class Picking(models.Model):
    ...
    approval_state = fields.Selection([('draft', 'Draft'),
                                       ...
                                       ('approved', 'Approved')], string='Approval Status', default='draft')
    ...
    def button_validate(self):
        if self.purchase_id:
            if any(self.move_lines.filtered(lambda line: line.quantity_done > (line.purchase_line_id.product_qty-line.purchase_line_id.qty_received))):
                if self.approval_state != 'approved' and not self.env.user.has_group("loyal_purchase_flow_control.group_po_grn_flow_approve"):
                    if self.approval_state == 'draft':
                        raise ValidationError(_("You can't validate this receipt due to done quantity exceeds orderd qty. Please send this for approval."))
                    if self.approval_state == 'waiting_for_approval':
                        raise ValidationError(
                            _("You can't validate this receipt due to done quantity exceeds orderd qty. Please wait for the approval."))
            if self.backorder_id and any(self.move_lines.filtered(lambda line: line.quantity_done > (line.product_uom_qty))):
                if not self.env.user.has_group("loyal_purchase_flow_control.group_po_grn_flow_approve"):
                    if self.approval_state in ('draft','approved'):
                        raise ValidationError(_("You can't validate this receipt due to done quantity exceeds orderd qty. Please send this for approval."))
                    if self.approval_state == 'waiting_for_approval':
                        raise ValidationError(
                            _("You can't validate this receipt due to done quantity exceeds orderd qty. Please wait for the approval."))
